...
button bexit = button(-0.5, -0.8, 0.8, 0.3, color(1, 0.1, 0.1), color(0, 0, 0), "EXIT");
...
void updateRotation() {
	...
	REP(a, rotationQueue.size()) {
		point mci = rotationQueue[a];
		int i = mci.z, j = mci.y, k = mci.x;
		Quaterniond qr = Quaterniond{ AngleAxisd{ degtorad(rorationSpeed), rotationAxis} }; // degree to radian
...
void displayHelp() {
	...
	printText(-3.2, -5.2, "1-9 : Choose the corresponding layer", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	...
	printText(-3.2, -6.6, "Scroll Up/Down : Rotate along Z-Axis", 1, GLUT_STROKE_ROMAN, colorList[1], 1);

}

void displayAbout() {

	printText(-3.0, 6.8, "LEARN HOW TO SOLVE", 3, GLUT_STROKE_ROMAN, color(1, 0.8, 0), 3);
	printText(-3.5, 6.5, "	               This is our college project. It utilizes ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 6.3, "		  to provide Rubik's Cube enthusiasts with a sophisticated tool for interacting ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 6.1, "	   with their favorite puzzle. The program enables smooth manipulation and rotation ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.9, "		 of the cube, accommodating large sizes up to 50x50x50, depending on available ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.7, "	memory and computational power. With its user-friendly interface, 3D buttons, and ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.5, "           seamless transitions, it offers an intuitive experience. ", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.3, "         Have fun exploring and experimenting with this project!", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
	printText(-3.5, 5.1, "     To Learn how to solve the RUBIK'S CUBE watch the following Link:", 1, GLUT_STROKE_ROMAN, colorList[3], 1);
